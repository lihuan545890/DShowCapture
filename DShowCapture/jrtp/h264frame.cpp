...
/*    PTRACE(4, "x264", "Profile: "   << (unsigned)payload[0] << 
...
bool H264Frame::EncapsulateFU(RTPFrame & frame, unsigned int & flags) {
  ...
  if ((m_currentNALFURemainingLen==0) || (m_currentNALFURemainingDataPtr==NULL))
  {
    ...
    //rtp FU Indicator ===> |F-1|NRI-2|FU-TYPE-5|(bit)
    m_currentNALFUHeader0 = (*m_currentNALFURemainingDataPtr & 0xe0) | 28;
    //rtp FU Header    ===> |S-1|E-1|R-1|NAL-TYPE-5|(bit)
...
bool H264Frame::DeencapsulateSTAP (RTPFrame & frame, unsigned int & /*flags*/) {
  ...
  //empty STAP-A NAL HDR or wrong STAP-A NAL HDR rtp packet
  if( curSTAPLen<=2 )   
  {
      //TRACE( "===========================curSTAPLen<=2================================" );
      return true;
...
//  PTRACE(6, "x264", "Deencapsulating a STAP of " << curSTAPLen << " bytes");
  while (curSTAPLen > 0)
  {
    ...
    //this len value include 1byte NRI header 
